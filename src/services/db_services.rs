use crate::services::extract_rows;
...
// pub async fn db_get_project(db: &Pool<Sqlite>, id: &str) -> Vec<String> { 
//
...
pub async fn db_get_exhibitions(db: &Pool<Sqlite>) -> Vec<Vec<String>> {
    ...
    let exhibitions_query = sqlx::query("SELECT title, fromm, till, link FROM exhibitions")
        .fetch_all(db)
        .await
        .expect("Failed to fetch exhibitions from db");
    
    let exhib_columns = vec!["title", "fromm", "till", "link"];
                // 0:"title", 1:"fromm", 2:"till" 3:"link"
